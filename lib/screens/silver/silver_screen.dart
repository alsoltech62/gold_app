...
class _SilverScreenState extends State<SilverScreen> {
  ...
  void _handleTransaction() async {
    ...
    if (_paymentMethod == 'RRFINCO') {
      ...
      if (response['success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(response['message'] ?? 'Could not create order'),
            backgroundColor: Colors.red,
          ),
        );
      }
    ...
    if (_paymentMethod != 'UPI' && _paymentMethod != 'RRFINCO') {
      Map<String, dynamic> result = await goldProvider.buySilver(
        amount,
        _paymentMethod,
        paymentId: 'WALLET_TXN',
      );
      if (!mounted) return;
      if (result['success'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Silver purchased successfully!')),
        );
        if (!mounted) return;
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (_) => const LockInScreen(metalType: 'silver'),
          ),
        );
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            const Text(
              'Please transfer exactly to the UPI ID below and enter the UTR/Reference ID to verify.',
              ...
              child: Row(
                children: [
                  ...
                  InkWell(
                    onTap: () {
                      Clipboard.setData(
                        const ClipboardData(text: 'goldbindia@oksbi'),
                      );
                      ScaffoldMessenger.of(context).showSnackBar(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              final result = await goldProvider.buySilver(
                amount,
                'UPI',
                paymentId: utrController.text,
              );
              if (!mounted) return;
              setState(() => _isProcessingPayment = false);
              ...
              if (result['success'] == true) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Silver purchased successfully!'),
                  ),
                );
                if (!mounted) return;
                Navigator.pushReplacement(
  ...
  void _showPaymentVerificationDialog(double amount, String orderId) {
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: const Text(
          'Did you complete the payment in the browser? Click below to verify and add silver to your vault.',
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              final result = await Provider.of<GoldProvider>(
                context,
                listen: false,
              ).buySilver(amount, 'RRFINCO', paymentId: orderId);
              if (!mounted) return;
              setState(() => _isProcessingPayment = false);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('SILVER VAULT'), centerTitle: true),
      body: Consumer<GoldProvider>(
        builder: (context, provider, child) {
          final rateRaw = provider.silverRate?['rate_per_gram'];
          final silverRate = rateRaw != null
              ...
              children: [
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${silverBalance.toStringAsFixed(4)} gms',
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 5),
                          Text(
                            currencyFormat.format(silverRate),
                            style: const TextStyle(
                              color: Colors.grey,
                              fontSize: 22,
                              fontWeight: FontWeight.bold,
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    DropdownMenuItem(
                      value: 'RRFINCO',
                      child: Text('Direct / UPI / Cards'),
                    ),
                    DropdownMenuItem(
                      value: 'UPI',
                      child: Text('Manual UPI Transfer'),
                    ),
                    DropdownMenuItem(
                      value: 'inr_wallet',
                      child: Text('INR Wallet Balance'),
                    ),
                    DropdownMenuItem(
                      value: 'japsan_wallet',
                      child: Text('Japsan Wallet Balance'),
                    ),
