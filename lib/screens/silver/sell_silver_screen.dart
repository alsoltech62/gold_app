...
class _SellSilverScreenState extends State<SellSilverScreen> {
  ...
  void _calculateAmount(String value) {
    ...
    final rate = Provider.of<GoldProvider>(context, listen: false).silverRate?['rate_per_gram'] ?? 0.0;
    if (rate > 0) {
  ...
  void _handleSell() async {
    ...
    final result = await goldProvider.sellSilver(grams);
    
    if (!mounted) return;
    
    if (result['success'] == true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Silver sell request submitted successfully!')),
      );
